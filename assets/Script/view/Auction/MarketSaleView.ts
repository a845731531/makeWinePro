...
@ccclass
export default class MarketSaleView extends BaseView {
    ...
    onEnable() {
        ...
        EventManager.instance.addEventListener("rspMarketBuyItem", this.onRspBuyItem, this)
    }
    ...
    initByExData(exData: any): void {
        this._isBuy = (exData && exData.isBuy)? true : false
    ...
    updateView(auctionData) {
        ...
        if (this._isSaleUp) {
            ...
            this.txtUnitPrice.string = this._auctionData.unitPrice / 1000 + ""

            this.txtAllPrice.string = this._auctionData.unitPrice * this._auctionData.count / 1000 + "" 
        ...
        this.btnBuy.active = this._isBuy 
        if(this._isBuy)
        {
    ...
    onEditBoxTextEnd() {
        ...
        if(!this._isSaleUp)
        {
            ...
            if(countStr != "" && unitPriceStr != "")
            {
                this.txtAllPrice.string = parseInt(unitPriceStr) * parseInt(countStr) + "" 
            }
        }
    ...
    onClickSale() {
        ...
        EventManager.instance.dispatchEvent("reqMarketShelfItem", {
            propId: this._itemData.propId,
            type: this._itemType,
            count: parseInt(countStr),
            unitPrice: parseInt(unitPriceStr) * 1000,
            totalPrice: parseInt(unitPriceStr) * 1000 * parseInt(countStr),
    ...
    onClickRetrieve() {
        if (this._auctionData) {
            EventManager.instance.dispatchEvent("reqMarketRetrieveItem", {
    ...
    onClickBuy() {
        ...
        if (this._auctionData && this._auctionData.totalPrice) {

            let needProce = this._auctionData.totalPrice
