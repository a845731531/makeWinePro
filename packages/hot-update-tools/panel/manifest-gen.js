...
Vue.component("manifest-gen", {
	...
	methods: {
		...
		onChangeSelectHotAddress(e) {
			...
			this.isShowUseAddrBtn = !0,
			this.isShowDelAddrBtn = !0,
			this._updateShowUseAddrBtn()
		},
		userLocalIP() {
		...
		onInPutUrlOver(e) {
			...
			if ( - 1 === s && -1 === i) { / ^([hH][tT] {
					...
					2
				} [pP][sS] : \ / \ / )(([A - Za - z0 - 9 - ~] + )\.) + ([A - Za - z0 - 9 - ~\ / ]) + $ / .test(t) || (this.log(t + " 不是以http://https://开头，或者不是网址, 已经自动修改"), this.serverRootDir = "http://" + this.serverRootDir, this._getRemoteServerVersion())
		...
		_saveConfig() {
			let e = {
				...
				genManifestDir: OutPut.manifestDir,
		...
		onInputVersionOver() {
			...
			null !== t && void 0 !== t && (this._isVersionPass(this.version, t) ? (this.log("上次构建时版本号: " + e), this._isVersionPass(this.version, e) ? this.log("版本通过验证!") : this.log("[Warning] 要构建的版本低于上次构建版本: " + this.version + "<=" + e)) : this.log("[Warning] version 填写的版本低于远程版本")),
			this._saveConfig()
		},
		onStopTouchEvent(e) {
			e.preventDefault(),
		...
		_genVersion(e, t, s, i) {
			...
			l = (e, t) = >{
				...
				h,
				d = Fs.readdirSync(e);
				for (let c = 0; c < d.length; ++c)"." !== d[c][0] && (i = Path.join(e, d[c]), (s = Fs.statSync(i)).isDirectory() ? l(i, t) : s.isFile() && (r = s.size, o = require("crypto").createHash("md5").update(Fs.readFileSync(i)).digest("hex"), a = ".zip" === Path.extname(i).toLowerCase(), h = (h = Path.relative(n, i)).replace(/\\/g, "/"), t[h = encodeURI(h)] = {
					size: r,
					md5: o
				},
				a && (t[h].compressed = !0)))
			...
			h = Path.join(o, "version.manifest"); (e = >{
				try {
					Fs.mkdirSync(e)
				} catch(e) {
		...
		_packageVersion() {
			...
			let n = Fs.readFileSync(s, "utf-8"),
			l = JSON.parse(n).version;
			...
			t.generateNodeStream({
				type: "nodebuffer",
				streamFiles: !0
			}).pipe(Fs.createWriteStream(d)).on("finish", () = >{
		...
		_checkResourceRootDir(e) {
			if (Fs.existsSync(e)) {
				...
				if (!Fs.existsSync(t)) return this.log(`没有发现$ {
					t
				},
				请先构建项目.`),
				!1;
				...
				return Util.manifestResDir ? (this.resourceRootDir = e, this._saveConfig(), !0) : (this.log(`没有发现资源目录$ {
					s.toString()
				},
				请先构建项目.`), !1)
			}
			return this.log(`没有发现$ {
				e
			},
			请先构建项目.`),
			!1
